...
import { warmCache, getCacheStats, clearExpiredCache, clearAllCache } from './cache/cache-cli.js';
...
async function main(): Promise<void> {
  ...
  if (command === 'run') {
    ...
    const isTimeWindow = TIME_WINDOWS.includes(windowArg as string);
  ...
  if (command === 'cache') {
    ...
    if (subcommand === 'warm') {
      ...
      try {
        console.log(
          `\nWarming genre cache for all Plex artists (concurrency: ${concurrency}, with retry & backoff)${dryRun ? ' [DRY RUN]' : ''}...\n`
        );
    ...
    if (subcommand === 'stats') {
      try {
        ...
        console.log(`\nExpiring within 7 days: ${stats.expiringWithin7Days}`);
    ...
    console.error('  warm   - Pre-populate cache for all Plex artists');
